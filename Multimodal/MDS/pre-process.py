...
for directory in os.listdir(os.path.join(opt.crop_dir,'real')):
	if os.path.isdir(os.path.join(opt.crop_dir,'real',directory)):
		if not os.path.exists(os.path.join(opt.tmp_dir,'real',directory)):
			...
			print("******************这里是6666**********************")
			command = ("ffmpeg -y -i %s -max_muxing_queue_size 9999 -qscale:v 2 -threads 1 -f image2 %s" % (videopath,os.path.join(framedir,'%06d.jpg'))) 
			output = subprocess.call(command, shell=True, stdout=None)
			print("******************这里是7777**********************")
			command = ("ffmpeg -y -i %s -max_muxing_queue_size 9999 -ac 1 -vn -acodec pcm_s16le -ar 48000 %s" % (videopath, os.path.join(audiodir,'audio.wav'))) 
			output = subprocess.call(command, shell=True, stdout=None)

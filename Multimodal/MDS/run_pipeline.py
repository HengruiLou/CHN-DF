...
def crop_video(track,cropfile):
  ...
  # ========== CROP AUDIO FILE ==========
  print("******************这里是1111**********************")
  command = ("ffmpeg -y -i %s -ss %.3f -max_muxing_queue_size 9999 -to %.3f %s" % (os.path.join(avi_dir,reference,'audio.wav'),audiostart,audioend,audiotmp)) 
...
def run_pipeline(arg_data_dir,arg_videofile,arg_reference,label):
  ...
  print("******************这里是3333**********************")
  command = ("ffmpeg -y -i %s -max_muxing_queue_size 9999 -qscale:v 2 -async 1 -r 30 %s" % (videofile,os.path.join(avi_dir,reference,'video.avi')))
  output = subprocess.call(command, shell=True, stdout=None)
  print("******************这里是4444**********************")
  command = ("ffmpeg -y -i %s -max_muxing_queue_size 9999 -qscale:v 2 -threads 1 -f image2 %s" % (os.path.join(avi_dir,reference,'video.avi'),os.path.join(frames_dir,reference,'%06d.jpg'))) 
  output = subprocess.call(command, shell=True, stdout=None)
  print("******************这里是5555**********************")
  command = ("ffmpeg -y -i %s -ac 1 -max_muxing_queue_size 9999 -vn -acodec pcm_s16le -ar 48000 %s" % (os.path.join(avi_dir,reference,'video.avi'),os.path.join(avi_dir,reference,'audio.wav'))) 
  output = subprocess.call(command, shell=True, stdout=None)

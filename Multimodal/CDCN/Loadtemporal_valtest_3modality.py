...
class Spoofing_valtest(Dataset):
    ...
    def __init__(self, root_dir,  transform=None):
        '''
        self.landmarks_frame_real = os.listdir(root_dir+"/FRAMS/true") #pd.read_csv(info_list, delimiter=' ', header=None)
        self.landmarks_frame_fake = os.listdir(root_dir+"/FRAMS/fake") #pd.read_csv(info_list, delimiter=' ', header=None)
        self.root_dir = root_dir
        self.transform = transform
        for xx in range( len(self.landmarks_frame_real)):
            self.landmarks_frame_real[xx] = root_dir+"/FRAMS/true/" + self.landmarks_frame_real[xx]
        for xx in range( len(self.landmarks_frame_fake)):
            self.landmarks_frame_fake[xx] = root_dir+"/FRAMS/fake/" + self.landmarks_frame_fake[xx]
        #self.landmarks_frame = self.landmarks_frame_fake + self.landmarks_frame_real



        self.landmarks_frame_real_s = os.listdir(root_dir+"/SPEC/true") #pd.read_csv(info_list, delimiter=' ', header=None)
        self.landmarks_frame_fake_s = os.listdir(root_dir+"/SPEC/fake") #pd.read_csv(info_list, delimiter=' ', header=None)
        for xx in range( len(self.landmarks_frame_real_s)):
            self.landmarks_frame_real_s[xx] = [self.landmarks_frame_real[xx], root_dir+"/SPEC/true/" + self.landmarks_frame_real_s[xx]]
        for xx in range( len(self.landmarks_frame_fake_s)):
            self.landmarks_frame_fake_s[xx] = [self.landmarks_frame_fake[xx],root_dir+"/SPEC/fake/" + self.landmarks_frame_fake_s[xx]]
        self.landmarks_frame = self.landmarks_frame_fake_s + self.landmarks_frame_real_s
        random.shuffle(self.landmarks_frame)
        '''
        self.root_dir = root_dir
        self.transform = transform
        self.landmarks_frame_real = Spoofing_valtest.get_image_paths(os.path.join(root_dir, "FRAMS/test/true"))
        self.landmarks_frame_fake = Spoofing_valtest.get_image_paths(os.path.join(root_dir, "FRAMS/test/fake"))
        self.landmarks_frame_real_s = Spoofing_valtest.get_image_paths(os.path.join(root_dir, "SPEC/test/true"))
        self.landmarks_frame_fake_s = Spoofing_valtest.get_image_paths(os.path.join(root_dir, "SPEC/test/fake"))
        for xx in range( len(self.landmarks_frame_real_s)):
